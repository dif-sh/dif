...
Anyone can force a variant by opening a `?_dif=` link, with no code and no
devtools. `initDif` reads it automatically (both `difLoad` server-side and the
client):

```
# force one experiment (persists for the tab session, then auto-clears)
https://staging.example.com/insights/foo?_dif=insights-cta-copy=variant_a
# multiple at once
…?_dif=insights-cta-copy=variant_a,home-hero=control
# clear
…?_dif=off
# generate the exact link from the CLI
dif qa --force insights-cta-copy=variant_a --preview-url https://staging.example.com/insights/foo
```
