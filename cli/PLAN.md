...
- **`dif` CLI**: built with `cargo dist`. Release artifacts cover macOS (arm64 + x86_64), Linux (x86_64 + aarch64, statically-linked musl so they run on any glibc baseline), and Windows (x86_64). Install via `curl | sh` (shipped script), the Homebrew tap, or `npm install -g @dif.sh/cli`. The Rust binary itself isn't on npm; the npm package (`@dif.sh/cli`, which *is* published) is a thin wrapper that downloads the matching binary on `postinstall` — keeping the CLI a single static binary while honoring the "npm install -g" pitch.
- **`@dif.sh/sdk`**: published to npm. ESM-only (`"type": "module"`), zero dependencies, no peer-deps beyond TypeScript types. The framework adapters `@dif.sh/react` and `@dif.sh/svelte` publish alongside it.
